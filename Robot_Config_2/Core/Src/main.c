...
// Encoder counters
int16_t signed_counter1 = 0;//L
int32_t signed_counter2 = 0;//R
int16_t signed_counter3 = 0;//B
...
//         float L_mm = calculateWheelDistance(signed_counter1,last_encoder1);
//         float R_mm = calculateWheelDistance(signed_counter2,last_encoder2);
//         float B_mm = calculateWheelDistance(signed_counter3,last_encoder3);
//         forward = (L_mm+R_mm)/2;//(R*ly-L*ry)/(ly-ry);
//         theta = (R_mm-L_mm)/(Ly+Ry);
//         strafe = B_mm - Bx*theta;
//         dforward = del(forward,lastforward);
//         dtheta = del(theta,lasttheta);
...
//         lastx = X;
//         lasty = Y;
//         last_encoder1 = signed_counter1;
//         last_encoder2 = signed_counter2;
//         last_encoder3 = signed_counter3;
//         lastforward = forward;
//         lasttheta = theta;
//         laststrafe = strafe;
...
void computeRobotMotion() {
    ...
    if (timeChange >= 0.01) {  // 10ms minimum interval
        ...
        // Calculate forward and strafe components
        forward = (L_mm + R_mm) / 2;//X is forward
        strafe = B_mm - Bx * dtheta;//Y is left
        ...
        if (fabs(dtheta) > 0.0001) {  // Avoid division by 0
            r0 = dforward / dtheta;
            r1 = dstrafe / dtheta;
        }
...
//
	...
	while (1) {
		...
		printf(
				"Received Integers: %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %d %ld\r\n",
				lx, ly, rx, ry, tri, cir, cro, squ, ll1, rr1, ll2, rr2, up,
				down, right, left, ebike_running, drib_speed);
